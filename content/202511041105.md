...
出典: [AWS Systems Manager Fleet Manager](https://docs.aws.amazon.com/systems-manager/latest/userguide/fleet.html)
...
2. 「デフォルトのホスト管理設定(DHMC)を有効にし、アカウント内の全EC2を自動で管理下にする」
...
必要な作業:
1. IAMロールを作成（信頼ポリシー: ec2.amazonaws.com）
2. AmazonSSMManagedInstanceCore ポリシーをアタッチ
3. インスタンスプロファイルを作成
4. EC2インスタンスにアタッチ
...
1. デフォルトIAMロール（AWSSystemsManagerDefaultEC2InstanceManagementRole）の作成
2. SSM Agentへの認証情報の自動提供
3. 条件を満たすEC2の自動登録
...
2. Account management → Configure Default Host Management Configuration
3. Enable Default Host Management Configuration をオン
